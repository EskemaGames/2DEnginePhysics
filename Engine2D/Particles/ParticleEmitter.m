...
- (id)initParticleEmitterWithFile:(NSString*)aFileName {
	...
	if (self != nil) {
		...
		//prepare the shared texture bound
...
- (void)parseParticleConfig:(TBXML*)aConfig {
	...
	NSString *fileData;
	if (element) {
		fileName = [TBXML valueOfAttributeNamed:@"name" forElement:element];
	...
	if (fileName && !fileData) {		
		...
		texture = [[Image alloc] initWithTexture:fileName  filter:GL_LINEAR Use32bits:YES TotalVertex:(maxParticles*12)];
	}
